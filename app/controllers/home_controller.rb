class HomeController < ApplicationController
  ...
  def index
    ...
    Rails.cache.fetch('@trending',{expires_in: 1.hours}) do
      ...
      response = HTTParty.get("#{ENV['API_URL']}/gifs/trending?api_key=#{ENV['KEY']}&limit=9")
  ...
  def search
    ...
    search_response = HTTParty.get("#{ENV['API_URL']}/gifs/search?q=#{search}&api_key=#{ENV['KEY']}&limit=9")
